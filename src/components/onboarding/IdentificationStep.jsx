            ...
            <div className="space-y-2">
              {[
                ...
                <div key={i} className="flex items-center gap-2">
                  <div
                    className="h-1.5 rounded-full bg-purple-300 overflow-hidden flex-1"
                    style={{ animationDelay: `${i * 0.3}s` }}
                  >
                    <div
                      className="h-full bg-gradient-to-r from-purple-500 to-blue-500 rounded-full"
                      style={{
                        animation: `progressBar 2s ease-in-out ${i * 0.4}s infinite alternate`,
                        width: '100%',
                      }}
                    />
                  </div>
                  <span className="text-xs text-purple-600 w-40 shrink-0">{step}</span>
                </div>
              ))}
            </div>
            <style>{`
              @keyframes progressBar {
                0% { transform: scaleX(0.1); transform-origin: left; opacity: 0.4; }
