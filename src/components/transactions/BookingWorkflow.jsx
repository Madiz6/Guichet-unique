...
const BOOKING_TEMPLATES = {
  'Dépense_Office & Administrative': (t) => [
  ...
  'Dépense_Professional Services': (t) => [
    { compte: '622', label: 'Honoraires', debit: t.amount || 0, credit: 0 },
    { compte: '445', label: 'TVA déductible', debit: t.tax_amount || 0, credit: 0 },
    { compte: '401', label: 'Fournisseurs', debit: 0, credit: t.total_amount || t.amount || 0 },
  ],
  'Dépense_Payroll & Benefits': (t) => [
  ...
  'Revenu_Product Sales / Services Revenue': (t) => [
...
export default function BookingWorkflow({ transaction, onBooked }) {
  const alreadyBooked = !!transaction.booking_status;

  const [step, setStep] = useState(alreadyBooked ? 4 : 1);
  const [expanded, setExpanded] = useState({ 1: !alreadyBooked, 2: false, 3: false, 4: alreadyBooked });
  ...
  const [paymentDone, setPaymentDone] = useState(!!transaction.payment_registered);
  const [loading, setLoading] = useState(false);

  const toggle = (n) => setExpanded(prev => ({ ...prev, [n]: !prev[n] }));

  // Save directly to DB and notify parent to refresh
  const save = async (fields) => {
    const updated = await base44.entities.Transaction.update(transaction.id, {
      ...transaction,
      ...fields,
    });
    onBooked(updated); // tell parent: refresh list + update selected
    return updated;
  };
  ...
  const handleGenerateEntries = async () => {
    setLoading(true);
    try {
      const templateFn = BOOKING_TEMPLATES[`${transaction.type}_${transaction.category}`] || DEFAULT_ENTRIES[transaction.type];
      let entries = templateFn ? templateFn(transaction) : [];

      // Try AI enhancement
      try {
        const result = await base44.integrations.Core.InvokeLLM({
          prompt: `Expert comptable Djiboutien. Génère écritures NPCG pour:
Type: ${transaction.type}, Description: ${transaction.description}
Montant HT: ${transaction.amount} DJF, TVA: ${transaction.tax_amount || 0} DJF
Total TTC: ${transaction.total_amount || transaction.amount} DJF, Paiement: ${transaction.payment_method || 'N/A'}
Retourne JSON: { entries: [{compte, label, debit, credit}] }. Débit = Crédit obligatoire.`,
          response_json_schema: {
            type: "object",
            properties: {
              entries: { type: "array", items: { type: "object", properties: { compte: { type: "string" }, label: { type: "string" }, debit: { type: "number" }, credit: { type: "number" } } } }
            }
          }
        });
        if (result?.entries?.length) entries = result.entries;
  ...
  const handleRegisterPayment = async () => {
    ...
    try {
      ...
      setPaymentDone(true);
      toast.success(`✅ Paiement enregistré — ${transaction.payment_method || 'Virement'}`);
    } catch {
    ...
    <div className="space-y-3">
      {alreadyBooked && (
        <div className="flex items-center justify-between p-3 bg-green-50 border border-green-200 rounded-lg">
          ...
          <Button type="button" size="sm" variant="ghost" onClick={handleReset} disabled={loading} className="text-gray-500 h-7">
        ...
        {expanded[1] && (
          <div className="pl-4 space-y-2">
            {[
              ...
              { key: 'Paie', label: '👥 Bulletin de paie', show: transaction.type === 'Dépense' },
              { key: 'Banque', label: '🏦 Relevé bancaire', show: true },
              { key: 'Autre', label: '📋 Autre écriture', show: true },
            ].filter(o => o.show).map(opt => (
              <button key={opt.key} onClick={() => handleSelectType(opt.key)}
      ...
      {/* Step 3 */}
      <div className="space-y-2">
        <StepHeader num={3} title="Passer l'écriture comptable" done={step > 3 || alreadyBooked} active={step === 3} />
          ...
          <div className="pl-4 space-y-3">
            <div className="bg-white border border-gray-200 rounded-lg overflow-hidden">
              <div className="bg-gray-50 px-3 py-2 text-xs font-semibold text-gray-600 flex justify-between border-b">
                <span>{transaction.description?.slice(0, 40)}</span>
                ...
                <tbody>
                  {journalEntries.map((entry, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="px-3 py-2 text-right font-medium">{entry.debit > 0 ? entry.debit.toLocaleString() : ''}</td>
                      <td className="px-3 py-2 text-right font-medium">{entry.credit > 0 ? entry.credit.toLocaleString() : ''}</td>
                    </tr>
                  ))}
            ...
            {!isBalanced && (
              <div className="flex items-center gap-2 p-2 bg-red-50 rounded text-xs text-red-700">
                <AlertCircle className="w-4 h-4" /> Écriture déséquilibrée
              </div>
            )}
            {isBalanced && <div className="text-xs text-green-700 flex items-center gap-1"><CheckCircle className="w-3 h-3" /> Écriture équilibrée</div>}
      ...
      {/* Step 4 */}
      <div className="space-y-2">
        <StepHeader num={4} title="Enregistrer le paiement" done={paymentDone || !!transaction.payment_registered} active={step === 4} />
        {expanded[4] && (
          <div className="pl-4 space-y-3">
            {paymentDone || transaction.payment_registered ? (
              ...
              <>
                <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg text-xs text-amber-800">
                  <strong>Solde à payer:</strong> {(transaction.total_amount || transaction.amount)?.toLocaleString()} DJF
                </div>
