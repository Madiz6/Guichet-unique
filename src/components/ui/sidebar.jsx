...
// ──────────────────────────────────────────────────────────────────────────────
// Simple tooltip primitives (avoid circular deps)
// ──────────────────────────────────────────────────────────────────────────────
const TooltipProvider = ({ children }) => <>{children}</>
const Tooltip = ({ children }) => <div className="relative group">{children}</div>
const TooltipTrigger = ({ children, asChild }) => <>{children}</>
const TooltipContent = ({ children, side, align, hidden, ...props }) => {
  if (hidden) return null
  return (
    <div className="absolute left-full ml-2 top-1/2 -translate-y-1/2 z-50 bg-black text-white text-xs px-2 py-1 rounded whitespace-nowrap opacity-0 group-hover:opacity-100 pointer-events-none transition-opacity">
      {children}
    </div>
  )
}
...
const SidebarProvider = React.forwardRef(function SidebarProvider(
  ...
  const setOpen = React.useCallback(
    function setOpenCallback(value) {
  ...
  const toggleSidebar = React.useCallback(
    function toggleSidebarCallback() {
      if (isMobile) {
        setOpenMobile(function(prev) { return !prev })
      } else {
...
// ──────────────────────────────────────────────────────────────────────────────
// SidebarHeader / Footer / Separator / Content / Group
// ──────────────────────────────────────────────────────────────────────────────
...
const SidebarMenuAction = React.forwardRef(function SidebarMenuAction(
  { className, asChild, showOnHover, ...props },
  ref
) {
  const _asChild = asChild === undefined ? false : asChild
  const _showOnHover = showOnHover === undefined ? false : showOnHover
  ...
  return (
    <Comp
      ...
      className={cn(
        ...
        "group-data-[collapsible=icon]:hidden",
        _showOnHover && "group-focus-within/menu-item:opacity-100 group-hover/menu-item:opacity-100 data-[state=open]:opacity-100 peer-data-[active=true]/menu-button:text-sidebar-accent-foreground md:opacity-0",
        className
      )}
      {...props}
    />
...
const SidebarMenuSkeleton = React.forwardRef(function SidebarMenuSkeleton({ className, showIcon, ...props }, ref) {
  const _showIcon = showIcon === undefined ? false : showIcon
  ...
  return (
    <div ref={ref} data-sidebar="menu-skeleton" className={cn("flex h-8 items-center gap-2 rounded-md px-2", className)} {...props}>
      {_showIcon && <Skeleton className="size-4 rounded-md" data-sidebar="menu-skeleton-icon" />}
      <Skeleton
...
const SidebarMenuSubButton = React.forwardRef(function SidebarMenuSubButton(
  { asChild, size, isActive, className, ...props },
  ref
) {
  const _asChild = asChild === undefined ? false : asChild
  const _size = size === undefined ? "md" : size
  const Comp = _asChild ? Slot : "a"
  return (
    <Comp
      ref={ref}
